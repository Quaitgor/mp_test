...
public class TextWriter extends Basic implements Observer {
	...
	public static TextWriter getInstance() {
		if (tw == null) {
			tw = new TextWriter();
			Iterator<String> keySetIterator = Controller.fonts.keySet()
					.iterator();
			while (keySetIterator.hasNext()) {
				String key = keySetIterator.next();
				JavaAndXML.getInstance().XMLtoJava(Controller.fonts.get(key),
						FontData.class);
			}
		}
		return tw;
	...
	private void writeInTFF(TextBlock tB, double delta) {
		...
		if (texts != null) {
			...
			for (int i = 0; i < texts.size(); i++) {
				...
				fonts.get(tB.getFont())
						.drawString(
								(float) x,
								(float) (y - (dx * fonts.get(tB.getFont())
										.getHeight())), line, Color.white);
			}
		}
	}
	/*
	...
	private void writeInSprite(TextBlock tB, double delta) {
		...
		// TODO here change textwriter lines depending on texts HashMap
		// Multilines!
		if (text != null) {
			x = tB.getX();
			y = tB.getY();
